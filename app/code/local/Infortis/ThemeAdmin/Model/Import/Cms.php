...
class Infortis_ThemeAdmin_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	/**
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/local/Infortis/ThemeAdmin/etc/import/';
    }
	...
	public function importCmsItems($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('themeadmin')->__("Can't read data file: %s", $xmlPath)
					);
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('themeadmin')->__('Number of imported items: %s', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('themeadmin')->__('No items were imported')
				);
			}
