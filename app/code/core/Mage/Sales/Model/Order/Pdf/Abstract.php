...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertLogo(&$page, $store = null)
    {
        ...
        if ($image) {
            ...
            if (is_file($image)) {
                ...
                $page->drawImage($image, 5, 680, 355, 915);
            }
        }
        //return $page;
    }
    ...
    protected function insertAddress(&$page, $store = null)
    {
        ...
        foreach (explode("\n", Mage::getStoreConfig('sales/identity/address', $store)) as $value){
            if ($value!=='') {
                $page->drawText(trim(strip_tags($value)), 150, $this->y, 'UTF-8');
                $this->y -=14;
            }
        }
        //return $page;
    ...
    protected function insertOrder(&$page, $order, $putOrderId = true)
    {
		...
		$headerTop = 820;
		$headerHeight = $headerTop - 53;
		$headerLeft = 350;

        $page->drawRectangle($headerLeft, $headerTop, 570, $headerHeight);

        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
		
		$lineHeight = $headerTop - 30;
		$headerLeftText = $headerLeft+10;


        if ($putOrderId) {
            $page->drawText(Mage::helper('sales')->__('Order # ').$order->getRealOrderId(), $headerLeftText, $lineHeight, 'UTF-8');
        }
		
		$lineHeight -= 14;
		
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('Order Date: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), $headerLeftText, $lineHeight, 'UTF-8');

        $page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
        $page->setLineColor(new Zend_Pdf_Color_GrayScale(0.5));
        $page->setLineWidth(0.5);
        $page->drawRectangle(25, 755, 275, 724);
        $page->drawRectangle(275, 755, 570, 724);
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
        $page->drawText(Mage::helper('sales')->__('SOLD TO:'), 35, 743 , 'UTF-8');

        if (!$order->getIsVirtual()) {
            $page->drawText(Mage::helper('sales')->__('SHIP TO:'), 285, 743 , 'UTF-8');
        }
        else {
        ...
        if (!$order->getIsVirtual()) {
            $y = 730 - (max(count($billingAddress), count($shippingAddress)) * 10 + 26);
        }
        else {
            $y = 730 - (count($billingAddress) * 10 + 26);
        }

        $page->setFillColor(new Zend_Pdf_Color_GrayScale(1));
        $page->drawRectangle(25, 740, 570, $y);
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
        $this->y = 726;
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
            $page->setLineWidth(0.5);
            $page->drawRectangle(25, $this->y, 275, $this->y-23);
            $page->drawRectangle(275, $this->y, 570, $this->y-23);
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->drawText($totalShippingChargesText, 285, $yShipments-14, 'UTF-8');
            $yShipments -=19;
            $tracks = $order->getTracksCollection();
            if (count($tracks)) {
                $page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
                $page->setLineWidth(0.5);
                $page->drawRectangle(285, $yShipments, 510, $yShipments - 20);
                $page->drawLine(380, $yShipments, 380, $yShipments - 20);
                // $page->drawLine(510, $yShipments, 510, $yShipments - 10);

                $this->_setFontRegular($page);
                $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
                //$page->drawText(Mage::helper('sales')->__('Carrier'), 290, $yShipments - 7 , 'UTF-8');
                $page->drawText(Mage::helper('sales')->__('Title'), 290, $yShipments - 14, 'UTF-8');
                $page->drawText(Mage::helper('sales')->__('Number'), 385, $yShipments - 14, 'UTF-8');
                ...
                foreach ($order->getTracksCollection() as $track) {
                    ...
                    $page->drawText($track->getNumber(), 390, $yShipments , 'UTF-8');
                    $yShipments -=10;
                }
            } else {
                $yShipments -= 10;
    ...
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array())
    {
        foreach ($draw as $itemsProp) {
            ...
            $height = isset($itemsProp['height']) ? $itemsProp['height'] : 14;
