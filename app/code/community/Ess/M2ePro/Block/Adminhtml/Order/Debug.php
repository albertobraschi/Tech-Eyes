...
class Ess_M2ePro_Block_Adminhtml_Order_Debug extends Mage_Adminhtml_Block_Widget_Container
{
    ...
    protected function _beforeToHtml()
    {
        ...
        if (!is_null($store->getId())) {
            $this->setData('store_tax_calculation_algorithm', $store->getConfig(Mage_Tax_Model_Config::XML_PATH_ALGORITHM));
            $this->setData('store_tax_calculation_based_on', $store->getConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_BASED_ON));
            $this->setData('store_price_includes_tax', $store->getConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_PRICE_INCLUDES_TAX));
            $this->setData('store_shipping_price_includes_tax', $store->getConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_SHIPPING_INCLUDES_TAX));

            $taxClass = Mage::getModel('tax/class')->load($store->getConfig(Mage_Tax_Model_Config::CONFIG_XML_PATH_SHIPPING_TAX_CLASS));
