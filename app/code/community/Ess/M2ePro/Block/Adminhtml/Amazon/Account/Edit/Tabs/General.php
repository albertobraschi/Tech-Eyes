    ...
    protected function _beforeToHtml()
    {
        ...
        if (Mage::helper('M2ePro')->getGlobalValue('temp_data') && Mage::helper('M2ePro')->getGlobalValue('temp_data')->getId()) {
            ...
            if (!$this->synchronizeProcessing) {
                ...
                Mage::helper('M2ePro')->setGlobalValue('temp_data', Mage::helper('M2ePro/Component_Amazon')->getModel('Account')->load($accountId));
            }
        } else {
            ...
            if (Mage::helper('M2ePro')->getGlobalValue('temp_data') && Mage::helper('M2ePro')->getGlobalValue('temp_data')->getId()) {
                ...
                if (!is_null($accountMarketplaceData)) {
                    $tempNewItem['locked'] = Mage::getModel('M2ePro/Template_General')->getCollection()
                                                                                      ...
                                                                                      ->addFieldToFilter('marketplace_id', $marketplaceObj->getId())
                                                                                      ->getSize();
                    ...
                    $tempNewItem['account_data']['marketplace_id'] = $accountMarketplaceData['marketplace_id'];
