...
class Ess_M2ePro_Block_Adminhtml_Ebay_Listing_Other_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        if (!is_null($this->getRequest()->getParam('ebayAccount'))) {
            $collection->addFieldToFilter('main_table.account_id', $this->getRequest()->getParam('ebayAccount'));
        }

        // Add Filter By Marketplace
        if (!is_null($this->getRequest()->getParam('ebayMarketplace'))) {
            $collection->addFieldToFilter('main_table.marketplace_id', $this->getRequest()->getParam('ebayMarketplace'));
        }
        ...
        $this->addColumn('actions', array(
            ...
            'actions'   => array(
                array(
                    ...
                    'url'       => array('base'=> '*/adminhtml_log/listingOther/back/'.Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_listingOther/index',array('tab'=>Ess_M2ePro_Block_Adminhtml_Component_Abstract::TAB_ID_EBAY)).'/'),
                    'field'     => 'id'
                ),
                array(
                    'caption'   => Mage::helper('M2ePro')->__('Clear Log'),
                    'url'       => array('base'=> '*/adminhtml_listingOther/clearLog/back/'.Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_listingOther/index',array('tab'=>Ess_M2ePro_Block_Adminhtml_Component_Abstract::TAB_ID_EBAY)).'/'),
                    'field'     => 'id',
    ...
    public function getViewLogIconHtml($ebayListingProductId)
    {
        ...
        $dbSelect = $this->connRead->select()
                                   ->from(Mage::getResourceModel('M2ePro/Listing_Other_Log')->getMainTable(),array('action_id','action','type','description','create_date','initiator'))
        ...
        $html .= '<a title="'.$iconTip.'" id="lpv_grid_help_icon_open_'.(int)$ebayListingProductId.'" href="javascript:void(0);" onclick="EbayListingItemGridHandlerObj.viewItemHelp('.(int)$ebayListingProductId.',\''.base64_encode(json_encode($actionsRows)).'\');"><img src="'.$iconSrc.'" /></a>';
        $html .= '<a title="'.$iconTip.'" id="lpv_grid_help_icon_close_'.(int)$ebayListingProductId.'" style="display:none;" href="javascript:void(0);" onclick="EbayListingItemGridHandlerObj.hideItemHelp('.(int)$ebayListingProductId.');"><img src="'.$iconSrc.'" /></a>';
        $html .= '</span>';
