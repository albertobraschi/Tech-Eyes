...
class Ess_M2ePro_Block_Adminhtml_Ebay_Account_Edit_Tabs_Feedback_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    {
        $value = '<a href="javascript:void(0);" onclick="EbayAccountHandlerObj.feedbacksOpenEditForm(\''.$row->getData('id').'\',\''.Mage::helper('M2ePro')->escapeJs($row->getData('body')).'\');">'.Mage::helper('M2ePro')->__('Edit').'</a>';
        return $value;
    }

    public function callbackActionDelete($value, $row, $column, $isExport)
    {
        $value = '<a href="javascript:void(0);" onclick="EbayAccountHandlerObj.feedbacksDeleteAction(\''.$row->getData('id').'\');">'.Mage::helper('M2ePro')->__('Delete').'</a>';
        return $value;
    }
