...
class Ess_M2ePro_Block_Adminhtml_Ebay_Feedback_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	...
	protected function _prepareCollection()
	{
        ...
        $collection->getSelect()
                   ...
                   ->joinLeft(
                       array('mlp' => Mage::getResourceModel('M2ePro/Ebay_Listing_Product')->getMainTable()),
                       '(`mlp`.ebay_item_id = `mei`.item_id)',
    ...
    public function callbackColumnEbayItemId($value, $row, $column, $isExport)
    {
        if (is_null($row->getData('listing_product_id'))) {
            ...
            $url = $this->getUrl('*/adminhtml_listing/goToEbay/', array('item_id' => $row->getData('ebay_item_id')));
            return '<a href="'.$url.'" target="_blank">'.$value.'</a>';
        }
    }
    ...
    public function callbackFilterFeedbackType($collection, $column)
    {
        ...
        switch ($value) {
            case Ess_M2ePro_Model_Ebay_Feedback::TYPE_NEGATIVE:
                $this->getCollection()->addFieldToFilter('buyer_feedback_type', Ess_M2ePro_Model_Ebay_Feedback::TYPE_NEGATIVE);
                break;
            case Ess_M2ePro_Model_Ebay_Feedback::TYPE_NEUTRAL:
                $this->getCollection()->addFieldToFilter('buyer_feedback_type', Ess_M2ePro_Model_Ebay_Feedback::TYPE_NEUTRAL);
                break;
            case Ess_M2ePro_Model_Ebay_Feedback::TYPE_POSITIVE:
                $this->getCollection()->addFieldToFilter('buyer_feedback_type', Ess_M2ePro_Model_Ebay_Feedback::TYPE_POSITIVE);
                break;
        }
    }
