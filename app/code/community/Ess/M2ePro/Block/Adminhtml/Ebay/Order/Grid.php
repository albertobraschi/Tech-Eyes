...
HTML;
            ...
            $itemUrl = Mage::helper('M2ePro/Component_Ebay')->getItemUrl($item->getItemId(), $row->getData('account_mode'), (int)$row->getData('marketplace_id'));
...
HTML;
    ...
    protected function callbackFilterPaymentCondition($collection, $column)
    {
        ...
        if ($value == 1) {
            $this->getCollection()->addFieldToFilter('payment_status', Ess_M2ePro_Model_Ebay_Order::PAYMENT_STATUS_COMPLETED);
        } else {
            $this->getCollection()->addFieldToFilter('payment_status', array('neq' => Ess_M2ePro_Model_Ebay_Order::PAYMENT_STATUS_COMPLETED));
        }
    }
    ...
    protected function callbackFilterShippingCondition($collection, $column)
    {
        ...
        if ($value == 1) {
            $this->getCollection()->addFieldToFilter('shipping_status', Ess_M2ePro_Model_Ebay_Order::SHIPPING_STATUS_COMPLETED);
        } else {
            $this->getCollection()->addFieldToFilter('shipping_status', array('neq' => Ess_M2ePro_Model_Ebay_Order::SHIPPING_STATUS_COMPLETED));
        }
    }
    ...
    {
        return $this->getUrl('*/adminhtml_ebay_order/view', array('id' => $row->getId(),'back'=>Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_order/index', array('tab' => Ess_M2ePro_Block_Adminhtml_Component_Abstract::TAB_ID_EBAY))));
    }
