...
class Ess_M2ePro_Block_Adminhtml_Listing_Log_Grid extends Ess_M2ePro_Block_Adminhtml_Log_Grid_Abstract
{
    ...
    protected function _prepareCollection()
    {
        ...
        if (count(Mage::helper('M2ePro/Component')->getEnabledComponents()) == 1) {
            if (Mage::helper('M2ePro/Component_Ebay')->isEnabled()) {
                $collection->getSelect()->where('component_mode = ? OR component_mode IS NULL',Ess_M2ePro_Helper_Component_Ebay::NICK);
            }
            if (Mage::helper('M2ePro/Component_Amazon')->isEnabled()) {
                $collection->getSelect()->where('component_mode = ? OR component_mode IS NULL',Ess_M2ePro_Helper_Component_Amazon::NICK);
            }
        }
    ...
    protected function _prepareColumns()
    {
        ...
        if (!isset($listingData['id']) && count(Mage::helper('M2ePro/Component')->getEnabledComponents()) > 1) {
    ...
    public function callbackColumnListingTitle($value, $row, $column, $isExport)
    {
        ...
        if ($row->getData('listing_id')) {
            $value = '<a href="'.$this->getUrl('*/adminhtml_'.$row->getData('component_mode').'_listing/view', array('id' => $row->getData('listing_id'))).'">'.
