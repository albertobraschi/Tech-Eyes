    ...
    public function getItems()
    {
        ...
        foreach ($collection as $account) {
            if (!isset($items[$account->getComponentMode()]['label'])) {
                $label = '';
                $account->isComponentModeEbay()   && $label = Mage::helper('M2ePro')->__(Ess_M2ePro_Helper_Component_Ebay::TITLE);
                $account->isComponentModeAmazon() && $label = Mage::helper('M2ePro')->__(Ess_M2ePro_Helper_Component_Amazon::TITLE);
