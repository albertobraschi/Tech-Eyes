        ...
        // MEDIA IMAGE
            ...
            if ($value == 'no_selection') {
                ...
                if (!preg_match('((mailto\:|(news|(ht|f)tp(s?))\://){1}\S+)',$value)) {
                    ...
                    $value = $this->prepareImageUrl($value);
                }
            }
        }
    ...
    protected function _getConfigurableOptionsForOrder()
    {
        ...
        foreach ($productAttributes as $attribute) {
            ...
            $configurableOption = array(
                ...
                'labels'    => array(
                    trim(strtolower($attribute['store_label'])),
                    trim(strtolower($attribute['frontend_label'])),
                    trim(strtolower($attribute['label'])),
                ),
                'values'    => array()
            );
            ...
            foreach ($attribute['values'] as $value) {
                $configurableOption['values'][] = array(
                    ...
                    'labels'   => array(
                        trim(strtolower($value['store_label'])),
                        trim(strtolower($value['default_label'])),
                        trim(strtolower($value['label']))
                    )
                );
            }
