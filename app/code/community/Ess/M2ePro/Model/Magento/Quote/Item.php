...
class Ess_M2ePro_Model_Magento_Quote_Item
{
    ...
    private function getAssociatedGroupedProduct()
    {
        ...
        $associatedProducts = Mage::getModel('M2ePro/Magento_Product')->setProduct($this->product)->getProductVariationsForOrder();
    ...
    private function calculateChannelCurrencyPrice()
    {
        ...
        if ($this->needToAddTax()) {
            $taxAmount = $taxCalculator->calcTaxAmount($this->channelCurrencyPrice, $this->proxyItem->getTaxRate(), false, false);
            $this->channelCurrencyPrice += $taxAmount;
        }

        if ($this->needToSubtractTax()) {
            $taxAmount = $taxCalculator->calcTaxAmount($this->channelCurrencyPrice, $this->proxyItem->getTaxRate(), true, false);
            $this->channelCurrencyPrice -= $taxAmount;
        }
    ...
    private function needToSubtractTax()
    {
        ...
        return $this->proxyItem->getTaxRate() != $taxCalculator->getStoreRate($request);
    }
