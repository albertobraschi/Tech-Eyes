...
class Ess_M2ePro_Model_Mysql4_Listing extends Ess_M2ePro_Model_Mysql4_Component_Parent_Abstract
{
    ...
    {
        $listingProductTable = Mage::getResourceModel('M2ePro/Listing_Product')->getMainTable();
        $listingProductVariationTable = Mage::getResourceModel('M2ePro/Listing_Product_Variation')->getMainTable();
        $listingProductVariationOptionTable = Mage::getResourceModel('M2ePro/Listing_Product_Variation_Option')->getMainTable();

        $dbSelect = $this->_getWriteAdapter()
                             ->select()
                             ->from(array('l' => $this->getMainTable()),new Zend_Db_Expr('DISTINCT `l`.`id`, `l`.`component_mode`'))
                             ->join(array('lp' => $listingProductTable),'`l`.`id` = `lp`.`listing_id`',array())
                             ->joinLeft(array('lpv' => $listingProductVariationTable),'`lp`.`id` = `lpv`.`listing_product_id`',array())
                             ->joinLeft(array('lpvo' => $listingProductVariationOptionTable),'`lpv`.`id` = `lpvo`.`listing_product_variation_id`',array())
