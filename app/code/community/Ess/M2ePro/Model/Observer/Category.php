        ...
        try {
            ...
            foreach ($addedProducts as $product) {
                ...
                foreach ($listingsModels as $listingModel) {
                    ...
                    //------------------------------
                    if ($listingModel->getData('categories_add_action') == Ess_M2ePro_Model_Listing::CATEGORIES_ADD_ACTION_NONE) {
                        continue;
                    }
                    //------------------------------
                    ...
                    // Only add product
                    //------------------------------
                    if ($listingModel->getData('categories_add_action') == Ess_M2ePro_Model_Listing::CATEGORIES_ADD_ACTION_ADD) {
                        $listingModel->addProduct($product instanceof Mage_Catalog_Model_Product ? $product : $productId);
                    }
                    //------------------------------

                    // Add product and list
                    //------------------------------
                    if ($listingModel->getData('categories_add_action') == Ess_M2ePro_Model_Listing::CATEGORIES_ADD_ACTION_ADD_LIST) {

                        /** @var $listingProduct Ess_M2ePro_Model_Listing_Product */
                        $listingProduct = $listingModel->addProduct($product instanceof Mage_Catalog_Model_Product ? $product : $productId);
            ...
            foreach ($deletedProducts as $product) {
                ...
                foreach ($listingsModels as $listingModel) {
                    ...
                    //------------------------------
                    if ($listingModel->getData('categories_delete_action') == Ess_M2ePro_Model_Listing::CATEGORIES_DELETE_ACTION_NONE) {
                        continue;
                    }
                    //------------------------------
                    ...
                    // Only stop product
                    //------------------------------
                    if ($listingModel->getData('categories_delete_action') == Ess_M2ePro_Model_Listing::CATEGORIES_DELETE_ACTION_STOP) {
                        $paramsTemp = array();
                        $paramsTemp['status_changer'] = Ess_M2ePro_Model_Listing_Product::STATUS_CHANGER_OBSERVER;
                    ...
                    //------------------------------
                    if ($listingModel->getData('categories_delete_action') == Ess_M2ePro_Model_Listing::CATEGORIES_DELETE_ACTION_STOP_REMOVE) {
                        $paramsTemp = array();
                        $paramsTemp['status_changer'] = Ess_M2ePro_Model_Listing_Product::STATUS_CHANGER_OBSERVER;
