    ...
    protected function getAddedVariations($magentoVariations, $currentVariations)
    {
        ...
        foreach ($magentoVariations as $mVariation) {
            ...
            if (!$isExistVariation) {
                ...
                if ((int)$cVariationExist['variation']['delete'] == Ess_M2ePro_Model_Listing_Product_Variation::DELETE_YES) {
    ...
    protected function addVariations(Ess_M2ePro_Model_Listing_Product $listingProduct, $addedVariations)
    {
        foreach ($addedVariations as $aVariation) {
            ...
            if (isset($aVariation['variation']['id'])) {
                ...
                Mage::helper('M2ePro/Component')->getComponentObject($this->componentMode,'Listing_Product_Variation',$aVariation['variation']['id'])
            ...
            $newVariationId = Mage::helper('M2ePro/Component')->getComponentModel($this->componentMode,'Listing_Product_Variation')
                                        ->addData($dataForAdd)
                                        ->save()
                                        ->getId();
            ...
            foreach ($aVariation['options'] as $aOption) {
                ...
                Mage::helper('M2ePro/Component')->getComponentModel($this->componentMode,'Listing_Product_Variation_Option')
                        ->addData($dataForAdd)
                        ->save();
            }
        }
    ...
    protected function markAsDeleteVariations($deletedVariations)
    {
        foreach ($deletedVariations as $dVariation) {
            if ($dVariation['variation']['status'] == Ess_M2ePro_Model_Listing_Product::STATUS_NOT_LISTED) {
                Mage::helper('M2ePro/Component')->getComponentObject($this->componentMode,'Listing_Product_Variation',$dVariation['variation']['id'])
                            ->deleteInstance();
            } else {
                $dataForUpdate = array(
