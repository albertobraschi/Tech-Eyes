...
class Ess_M2ePro_Model_Ebay_Listing_Product extends Ess_M2ePro_Model_Component_Child_Ebay_Abstract
{
    ...
    public function getInstanceByEbayItem($ebayItem)
    {
        // Prepare tables names
        //-----------------------------
        $ebayItemsTable = Mage::getResourceModel('M2ePro/Ebay_Item')->getMainTable();
        //-----------------------------

        // Get listing product
        //-----------------------------
        $collection = Mage::getModel('M2ePro/Ebay_Listing_Product')->getCollection();
        $collection->getSelect()->join(array('mei' => $ebayItemsTable), '(main_table.ebay_item_id = mei.id AND mei.item_id = ' . $ebayItem . ')', array());
        //-----------------------------
    ...
    {
        $src = $this->getEbayGeneralTemplate()->getLocalShippingCashOnDeliveryCostMode();
    ...
    public function getTitle()
    {
        ...
        switch ($src['mode']) {
            ...
            case Ess_M2ePro_Model_Ebay_Template_Description::TITLE_MODE_CUSTOM:
                $title = Mage::getSingleton('M2ePro/Template_Description_Parser')->parseTemplate($src['template'], $this->getMagentoProduct()->getProduct());
                break;
    ...
    public function getSubTitle()
    {
        ...
        if ($src['mode'] == Ess_M2ePro_Model_Ebay_Template_Description::SUBTITLE_MODE_CUSTOM) {
            $subTitle = Mage::getSingleton('M2ePro/Template_Description_Parser')->parseTemplate($src['template'], $this->getMagentoProduct()->getProduct());
            if ($this->getEbayDescriptionTemplate()->isCutLongTitles()) {
    ...
    public function getDescription()
    {
        ...
        switch ($src['mode']) {
            case Ess_M2ePro_Model_Ebay_Template_Description::DESCRIPTION_MODE_PRODUCT:
                ...
                $description = $templateProcessor->filter(nl2br($description));
                break;
            ...
            case Ess_M2ePro_Model_Ebay_Template_Description::DESCRIPTION_MODE_CUSTOM:
                $description = Mage::getSingleton('M2ePro/Template_Description_Parser')->parseTemplate($src['template'], $this->getMagentoProduct()->getProduct());
                break;
        ...
        switch ($mode) {
            ...
            case Ess_M2ePro_Model_Ebay_Template_SellingFormat::PRICE_SPECIAL:
                if ($this->getMagentoProduct()->isGroupedType()) {
                    $price = $this->getBaseGroupedProductPrice(Ess_M2ePro_Model_Ebay_Template_SellingFormat::PRICE_SPECIAL);
                } else {
            ...
            case Ess_M2ePro_Model_Ebay_Template_SellingFormat::PRICE_PRODUCT:
                if ($this->getMagentoProduct()->isGroupedType()) {
                    $price = $this->getBaseGroupedProductPrice(Ess_M2ePro_Model_Ebay_Template_SellingFormat::PRICE_PRODUCT);
                } else {
    ...
    public function getWeight()
    {
        ...
        if ($src['mode'] == Ess_M2ePro_Model_Ebay_Template_General_CalculatedShipping::WEIGHT_CUSTOM_ATTRIBUTE) {
            ...
            if (count($weight_array) >= 2) {
                ...
                if ($minor > 0 && $this->getEbayGeneralTemplate()->getCalculatedShipping()->isMeasurementSystemEnglish()) {
    ...
    {
        return Mage::getModel('M2ePro/Connector_Server_Ebay_Item_Dispatcher')->process($action, $this->getId(), $params);
    }
