...
class Ess_M2ePro_Model_Amazon_Order extends Ess_M2ePro_Model_Component_Child_Amazon_Abstract
{
    ...
    public function getBuyerEmail()
    {
        ...
        if ($email == '') {
            $email = str_replace(' ', '-', strtolower($this->getBuyerName())) . Ess_M2ePro_Model_Magento_Customer::FAKE_EMAIL_POSTFIX;
        }
    ...
    /**
     * Merges separate items mapped to same products
     *
     * @return null
     */
    private function mergeItems()
    {
        if ($this->getParentObject()->isSingle()) {
            return;
        }

        foreach ($this->getParentObject()->getItemsCollection()->getItems() as $comparedItem) {

            foreach ($this->getParentObject()->getItemsCollection()->getItems() as $currentItem) {
                if ($comparedItem->getId() == $currentItem->getId() ||
                    $comparedItem->getProductId() != $currentItem->getProductId() ||
                    $comparedItem->getPrice() != $currentItem->getPrice()) {
                    continue;
                }

                $mergedQty = $comparedItem->getQtyPurchased() + $currentItem->getQtyPurchased();
                $comparedItem->setData('qty_purchased', (int)$mergedQty)->save();

                $this->getParentObject()->getItemsCollection()->removeItemByKey($currentItem->getData('order_item_id'));
                $currentItem->deleteInstance();
                break;
            }
        }
    }
    ...
    public function canCreateMagentoOrder()
    {
        ...
        // M2E order
        // ---------------
        if ($this->getParentObject()->hasListingItems() && !$amazonAccount->isMagentoOrdersListingsModeEnabled()) {
            return false;
        }
        // ---------------
        ...
        // 3rd party order
        // ---------------
        if ($this->getParentObject()->hasOtherListingItems() && !$amazonAccount->isMagentoOrdersListingsOtherModeEnabled()) {
            return false;
        }
        // ---------------
    ...
    private function processDispatcher(array $params)
    {
        /** @var $dispatcher Ess_M2ePro_Model_Connector_Xfabric_Amazon_Dispatcher */
        $dispatcher = Mage::getModel('M2ePro/Connector_Xfabric_Amazon_Dispatcher');
        $dispatcher->processConnector('orders', 'update', 'items', $params,
    ...
    public function updateShippingStatus(array $trackingDetails = array())
    {
        ...
        if (!empty($trackingDetails['tracking_number'])) {
            ...
            $carrier = isset($supportedCarriers[$trackingDetails['carrier_code']])
                ? $supportedCarriers[$trackingDetails['carrier_code']]
