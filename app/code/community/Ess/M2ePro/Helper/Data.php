...
class Ess_M2ePro_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public static function arrayReplaceRecursive(array $target, array $replacement)
    {
        if (function_exists('array_replace_recursive')) {
            return array_replace_recursive($target, $replacement);
        }
        ...
        foreach ($replacement as $key => $value) {
            if (is_int($key)) {
                !in_array($target, $value) && $target[] = $value;
                continue;
            }
            ...
            !array_key_exists($key, $target) && $target[$key] = NULL;
            $target[$key] = self::arrayReplaceRecursive($target[$key], $replacement[$key]);
        }
    ...
    {
        $globalKey = self::CUSTOM_IDENTIFIER.'_'.$key;
        return Mage::register($globalKey,$value,!Mage::helper('M2ePro/Server')->isDeveloper());
    }
