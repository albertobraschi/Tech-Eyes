...
class Zizio_Powershare_Block_Adminhtml_Incentives_Edit_Categories_Tree extends Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Categories
{
	...
	public function getField($elementId, $type, $config=array())
	{
		...
		$className = 'Zizio_Powershare_Model_Form_Element_'.$type;
        try {
			...
			$className = 'Varien_Data_Form_Element_'.$type;
			$element = new $className($config);
		}
	...
	{
		$cat = $this->getField("all_categories", "checkbox", array(
			'name'     => 'all_categories',
        ...
        $hr = $this->getField("product-categories-hr", "hr");
